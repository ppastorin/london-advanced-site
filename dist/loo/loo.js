...
$("#map-toggle").onclick=()=>{const w=$("#map-wrap"),show=w.hidden;w.hidden=!show;$("#map-toggle").textContent=show?(IT?"Nascondi mappa":"Hide map"):(IT?"Mostra mappa":"Show map");$("#map-toggle").setAttribute("aria-expanded",String(show));if(show&&state.map)setTimeout(()=>state.map.invalidateSize(),30)};
function initAboutPanel(){const panel=$("#loo-about-panel");if(!panel)return;const close=panel.querySelector(".panel-close"),triggers=document.querySelectorAll("[data-loo-about]");if(!close||!triggers.length)return;const open=()=>{if(!panel.open)panel.showModal()};triggers.forEach(trigger=>trigger.addEventListener("click",open));close.addEventListener("click",()=>panel.close());panel.addEventListener("click",event=>{const bounds=panel.getBoundingClientRect(),inside=event.clientX>=bounds.left&&event.clientX<=bounds.right&&event.clientY>=bounds.top&&event.clientY<=bounds.bottom;if(!inside)panel.close()});if(new URLSearchParams(window.location.search).get("about")==="1")open()}
initAboutPanel();load().catch(()=>{});
